...
- Dice rolls carry neither text nor a downloadable file. They fold into the message body as a one-line marker (`[Dice 🎲 = 4]`), so rolls made in the chat reach the agent; the envelope supplies the sender as it does for any other message.
